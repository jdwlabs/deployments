...
#   gh api repos/jdwlabs/deployments/rulesets/<id> | jq . > <name>.json
#
...
#
# Related: .github/workflows/promote-prd.yml hardcodes required-check job
# names ("lint", "template", "validate-config") into a generated PR body —
# update that in the same lockstep if the renamed/removed job is one of them.

set -euo pipefail

REPO="jdwlabs/deployments"
